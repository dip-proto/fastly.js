...
describe("Table Functions Tests", () => {
	it("should create a table and add entries", () => {
		...
		expect(context.std.table.add_entry("features", "new_ui", "false")).toBe(
			true,
		);
		expect(context.std.table.add_entry("features", "max_items", 10)).toBe(true);
		expect(context.std.table.add_entry("features", "discount_rate", 0.15)).toBe(
			true,
		);
		expect(context.std.table.add_entry("features", "is_enabled", true)).toBe(
			true,
		);
		expect(
			context.std.table.add_entry("features", "url_pattern", "^/api/v[0-9]+"),
		).toBe(true);
	...
	it("should remove an entry from a table", () => {
		...
		// Remove an entry
		expect(context.std.table.remove_entry("features", "new_checkout")).toBe(
			true,
		);
		...
		expect(context.std.table.remove_entry("features", "nonexistent")).toBe(
			false,
		);
	});
	...
	it("should look up a key in a table and return its value as a boolean", () => {
		...
		expect(context.std.table.lookup_bool("features", "string_false")).toBe(
			false,
		);
		expect(context.std.table.lookup_bool("features", "number_zero")).toBe(
			false,
		);
		expect(context.std.table.lookup_bool("features", "number_nonzero")).toBe(
			true,
		);
		...
		expect(context.std.table.lookup_bool("features", "nonexistent")).toBe(
			false,
		);
	...
	it("should look up a key in a table and return its value as an integer", () => {
		...
		expect(context.std.table.lookup_integer("settings", "string_integer")).toBe(
			42,
		);
		expect(context.std.table.lookup_integer("settings", "string_float")).toBe(
			3,
		); // Truncated
		expect(context.std.table.lookup_integer("settings", "number_integer")).toBe(
			42,
		);
		expect(context.std.table.lookup_integer("settings", "number_float")).toBe(
			3,
		); // Floored
		...
		expect(
			context.std.table.lookup_integer("settings", "nonexistent", 100),
		).toBe(100);
		expect(context.std.table.lookup_integer("nonexistent", "key")).toBe(0);
		expect(context.std.table.lookup_integer("nonexistent", "key", 100)).toBe(
			100,
		);
	});
	...
	it("should look up a key in a table and return its value as a float", () => {
		...
		expect(context.std.table.lookup_float("settings", "string_integer")).toBe(
			42.0,
		);
		expect(context.std.table.lookup_float("settings", "string_float")).toBe(
			3.14,
		);
		expect(context.std.table.lookup_float("settings", "number_integer")).toBe(
			42.0,
		);
		expect(context.std.table.lookup_float("settings", "number_float")).toBe(
			3.14,
		);
	...
	it("should look up a key in a table and return its value as a regex", () => {
		...
		const nonexistentRegex = context.std.table.lookup_regex(
			"patterns",
			"nonexistent",
		);
		const defaultRegex = context.std.table.lookup_regex(
			"patterns",
			"nonexistent",
			"abc",
		);
