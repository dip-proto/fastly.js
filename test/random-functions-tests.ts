  ...
  tests: [
    // Test 1: Basic random boolean generation
    {
      ...
      assertions: [
        ...
        (context: VCLContext) => {
          return assert(
            context.req.http['X-Random-50'] === 'true' || context.req.http['X-Random-50'] === 'false',
    ...
    // Test 3: Random integer generation
    {
      ...
      assertions: [
        // Check range 1-10
        (context: VCLContext) => {
          const value = parseInt(context.req.http['X-Random-1-10']);
          return assert(
            value >= 1 && value <= 10,
        ...
        (context: VCLContext) => {
          const value = parseInt(context.req.http['X-Random-Negative']);
          return assert(
            value >= -10 && value <= -1,
