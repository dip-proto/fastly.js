	...
	tests: [
		...
		},
		{
			...
			run: async (context: VCLContext, subroutines: VCLSubroutines) => {
				...
				const fetchResult = executeSubroutine(
					context,
					subroutines,
					"vcl_fetch",
				);
		...
		},
		{
			...
			assertions: [
				...
				(context: VCLContext) => {
					return assert(
						context.req.http["X-AB-Test"] === "A" ||
							context.req.http["X-AB-Test"] === "B",
				...
				(context: VCLContext) => {
					return assert(
						context.req.http.Cookie?.includes("ab_test="),
		...
		},
		{
			...
			assertions: [
				...
				(context: VCLContext) => {
					return assert(
						context.resp.http["Content-Security-Policy"]?.includes(
							"default-src",
						),
		...
		},
		{
			...
			run: async (context: VCLContext, subroutines: VCLSubroutines) => {
				...
				const errorResult = executeSubroutine(
					context,
					subroutines,
					"vcl_error",
				);
			...
			assertions: [
				...
				(context: VCLContext) => {
					return assert(
						context.obj.response && typeof context.obj.response === "string",
				...
				(context: VCLContext) => {
					return assert(
						context.obj.response?.includes("API Key Required"),
		...
		},
		{
			...
			assertions: [
				...
				(context: VCLContext) => {
					return assert(
						context.obj.response?.includes("Too Many Requests"),
