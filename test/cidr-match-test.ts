...
describe("CIDR Matching Tests", () => {
	...
	it("should correctly match IPv6 addresses in CIDR ranges", () => {
		...
		expect(context.std!.acl!.check("2001:db8:ffff:ffff:ffff:ffff:ffff:ffff", "test_ipv6")).toBe(
			true,
		);
		...
		expect(context.std!.acl!.check("febf:ffff:ffff:ffff:ffff:ffff:ffff:ffff", "test_ipv6")).toBe(
			true,
		);
