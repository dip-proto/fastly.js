  ...
  tests: [
    // Test 1: Time arithmetic functions
    {
      ...
      assertions: [
        // Check time.add
        (context: VCLContext) => {
          const baseTime = parseInt(context.req.http['X-Base-Time']);
          const timePlus1Hour = parseInt(context.req.http['X-Time-Plus-1Hour']);
          const timePlus1Day = parseInt(context.req.http['X-Time-Plus-1Day']);
        ...
        (context: VCLContext) => {
          const baseTime = parseInt(context.req.http['X-Base-Time']);
          const timeMinus1Hour = parseInt(context.req.http['X-Time-Minus-1Hour']);
          const timeDiff = parseInt(context.req.http['X-Time-Diff']);

          return assert(
            timeMinus1Hour === baseTime - 3600 &&
            timeDiff === 3600,
    ...
    // Test 2: Time formatting and parsing
    {
      ...
      assertions: [
        ...
        (context: VCLContext) => {
          const baseTime = parseInt(context.req.http['X-Base-Time']);
          const parsedTime = parseInt(context.req.http['X-Parsed-Time']);
