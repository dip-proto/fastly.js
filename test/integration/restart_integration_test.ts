  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      if (restartCount !== testCase.expectedRestarts) {
        errors.push(`Restart count mismatch: expected ${ testCase.expectedRestarts }, got ${ restartCount }`);
        passed = false;
      }
      ...
      if (testCase.name === 'URL Normalization' &&
        ...
        errors.push(`X-Current-URL header mismatch: expected "${ testCase.expectedHeaders['X-Current-URL'] }", got "${ context.req.http['X-Current-URL'] }"`);
        passed = false;
      }
      ...
      if (testCase.expectedBackend && context.req.backend !== testCase.expectedBackend) {
        errors.push(`Backend mismatch: expected ${ testCase.expectedBackend }, got ${ context.req.backend }`);
        passed = false;
      }
