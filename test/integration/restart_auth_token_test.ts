...
const subroutines = loadVCL('./examples/restart/auth_token_validation.vcl');
  ...
  for (const testCase of testCases) {
    ...
    processRequest().then(() => {
      ...
      if ((context.req.http['X-User-Role'] || '') !== testCase.expectedRole) {
        console.log(`❌ Role mismatch: expected ${testCase.expectedRole}, got ${context.req.http['X-User-Role'] || 'none'}`);
        passed = false;
      }
