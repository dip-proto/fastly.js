...
function testSimpleRestart() {
  ...
  if (context.req.http['X-Custom-Header'] !== expectedHeader) {
    console.log(`❌ Header mismatch: expected "${ expectedHeader }", got "${ context.req.http['X-Custom-Header'] }"`);
    passed = false;
  }

  if (context.req.http['X-Restart-Reason'] !== expectedReasons[2]) {
    console.log(`❌ Restart reason mismatch: expected "${ expectedReasons[2] }", got "${ context.req.http['X-Restart-Reason'] }"`);
    passed = false;
  }
