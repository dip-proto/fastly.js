...
console.log(`Default backend: ${JSON.stringify(context.backends.default!, null, 2)}`);
console.log(`Is default backend healthy? ${context.std!.backend!.is_healthy("default")}`);
...
console.log(`Static backend: ${JSON.stringify(context.backends.static!, null, 2)}`);
...
console.log(`API backend with probe: ${JSON.stringify(context.backends.api!, null, 2)}`);
...
console.log(`Current backend object: ${JSON.stringify(context.current_backend, null, 2)}`);

// Test removing a backend
console.log("\nRemoving Backend Test");
console.log(`Backends before removal: ${Object.keys(context.backends).join(", ")}`);
context.std!.backend!.remove("static");
console.log(`Backends after removal: ${Object.keys(context.backends).join(", ")}`);
...
console.log(`Created director: ${JSON.stringify(context.directors.main_director!, null, 2)}`);
...
console.log(`Director with backends: ${JSON.stringify(context.directors.main_director!, null, 2)}`);
...
for (let i = 0; i < 10; i++) {
	...
	console.log(`Selection ${i + 1}: ${selectedBackend ? selectedBackend.name : "none"}`);
}
...
console.log(`Fallback director: ${JSON.stringify(context.directors.fallback_director!, null, 2)}`);
...
const fallbackBackend2 = context.std!.director!.select_backend("fallback_director");
