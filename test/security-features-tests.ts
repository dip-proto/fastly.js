  ...
  tests: [
    // Test 1: SQL Injection Detection
    {
      ...
      run: async (context: VCLContext, subroutines: VCLSubroutines) => {
        ...
        try {
          ...
          if (!error.message.includes('Forbidden: Suspicious SQL patterns detected')) {
            throw new Error(`Expected SQL injection error, got: ${ error.message }`);
          }
        }
      },
      assertions: [
    ...
    // Test 2: XSS Detection
    {
      ...
      run: async (context: VCLContext, subroutines: VCLSubroutines) => {
        ...
        try {
          ...
          // Expected to throw an error due to security block
          if (!error.message.includes('Forbidden: Suspicious XSS patterns detected')) {
    ...
    // Test 3: Path Traversal Detection
    {
      ...
      run: async (context: VCLContext, subroutines: VCLSubroutines) => {
        ...
        try {
          ...
          // Expected to throw an error due to security block
          if (!error.message.includes('Forbidden: Path traversal attempt detected')) {
    ...
    // Test 4: Security Headers
    {
      ...
      assertions: [
        ...
        (context: VCLContext) => {
          return assert(
            context.resp.http['Strict-Transport-Security'] === 'max-age=31536000; includeSubDomains',
